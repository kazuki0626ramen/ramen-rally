    ...
    const { data, error } = await supabase
      ...
      .select(`
        *,
        ...
        stamps(count),
        likes(count),
        comments(id, content, created_at, profiles:user_id(nickname))
      `)
      .eq("is_public", true)
              ...
              <div key={post.id} className="bg-white rounded-[32px] overflow-hidden shadow-lg shadow-orange-100/20 border border-white">
                ...
                <div className="p-5">
                  <div className="flex justify-between items-start mb-2">
                    <h3 className="font-black text-slate-800 tracking-tight text-lg">@{(post.shops as any)?.name || "Shop"}</h3>
                  ...
                  <div className="flex gap-6 border-t border-slate-50 pt-4 mb-4">
                    ...
                    <button 
                      ...
                      className="flex items-center gap-1.5 text-slate-600 font-black text-xs"
                    >
                    ...
                    <div className="space-y-3 mb-4 bg-slate-50 p-4 rounded-2xl border border-slate-100">
                      {post.comments.map((c: any) => (
                        <div key={c.id} className="text-xs">
                          <span className="font-black text-slate-800 mr-2">{c.profiles?.nickname}:</span>
                  ...
                  {showCommentInput[post.id] && (
                    <div className="flex gap-2 animate-in fade-in slide-in-from-top-2">
                      <input 
                        ...
                        placeholder="おいしそう！"
                        className="flex-1 bg-slate-100 rounded-full px-4 py-2 text-xs font-medium text-slate-900 focus:outline-none focus:ring-2 focus:ring-orange-100"
                      />
                      <button 
                        onClick={() => handleSendComment(post.id)}
                        className="bg-orange-500 text-white px-4 py-2 rounded-full text-xs font-black shadow-md shadow-orange-100 active:scale-95 transition-all"
                      >
                        送信
                      </button>
                    </div>
