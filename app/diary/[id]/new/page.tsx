...
export default function NewDiaryPage() {
  ...
  const [imageUrl, setImageUrl] = useState(""); // ※本来はここでStorageへアップロード
  const [loading, setLoading] = useState(false);

  // スタンプ帳の「📝 Log」から来た場合に店名を自動セット
  useEffect(() => {
  ...
  return (
    <main className="p-6 bg-[#FFF9F5] min-h-screen font-sans flex flex-col items-center">
      ...
      <form onSubmit={handleSubmit} className="w-full max-w-md space-y-6">
        ...
        {/* 簡易的な写真URL入力（本来はここにFile Uploadが入ります） */}
        <div className="bg-white p-6 rounded-[32px] shadow-sm border border-orange-50">
        ...
        <div className="bg-white p-6 rounded-[32px] shadow-sm border border-orange-50 flex items-center justify-between">
          <div>
            ...
            <p className="text-[10px] text-slate-300 font-bold mt-1">{isPublic ? "タイムラインに公開されます" : "自分だけが見れる状態です"}</p>
          </div>
          <button 
            type="button"
        ...
        <button 
          type="submit" 
          disabled={loading}
          className="w-full bg-orange-600 text-white font-black py-4 rounded-[32px] shadow-xl active:scale-95 transition-all disabled:opacity-50"
        >
