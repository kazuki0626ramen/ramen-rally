...
export default function DiaryPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  const handleImageUpload = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    setSaving(true);
    const { error: uploadError } = await supabase.storage.from("ramen_images").upload(fileName, file);
    if (uploadError) {
      alert("アップロード失敗");
      setSaving(false);
      return;
    }
    const { data } = supabase.storage.from("ramen_images").getPublicUrl(fileName);
